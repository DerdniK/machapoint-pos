    ...
    options.ForwardDefaultSelector = context =>
    {
        ...
        if (handler.CanReadToken(tokenStr))
        {
            try
            {
                ...
                // Token emitido por Supabase (Google OAuth / ES256)
        ...
        // Token emitido localmente por ServicioUsers (HS256)
        return "LocalAuth";
    };
})
// Validador 1: Tokens locales (ServicioUsers - HS256)
.AddJwtBearer("LocalAuth", options =>
{
    ...
    options.Events = new JwtBearerEvents
    {
        ...
        {
            Console.WriteLine($"[LocalAuth FAIL - ServicioShift] {context.Exception.Message}");
            return Task.CompletedTask;
        },
        OnTokenValidated = context =>
        {
            Console.WriteLine($"[LocalAuth OK - ServicioShift] Token de usuario local validado.");
            return Task.CompletedTask;
        }
    };
})
...
.AddJwtBearer("SupabaseAuth", options =>
{
    ...
    options.Events = new JwtBearerEvents
    {
        OnTokenValidated = context =>
        {
            ...
            Console.WriteLine($"[SupabaseAuth OK - ServicioShift] Token Supabase validado y rol Admin asignado.");
            return Task.CompletedTask;
        },
        OnAuthenticationFailed = context =>
        {
            Console.WriteLine($"[SupabaseAuth FAIL - ServicioShift] {context.Exception.Message}");
            return Task.CompletedTask;
        }
    };
});

// Políticas de autorización adaptadas al ciclo de vida de turnos y arqueos
builder.Services.AddAuthorization(options =>
{
    // Cierres de caja forzados, auditorías o configuraciones de terminales
    options.AddPolicy("AdminOnly", policy => 
        policy.RequireRole("Admin", "authenticated"));

    // Apertura, registro de movimientos y cierre de turno regular
    options.AddPolicy("ShiftOperators", policy => 
        policy.RequireRole("Cajero", "Admin", "authenticated"));

    // Supervisores o administradores para autorizaciones especiales (retiros, arqueos ciegos)
