...
// Conexión a Base de Datos (PostgreSQL en Supabase/RDS)
...
.AddJwtBearer("LocalAuth", options =>
{
    ...
    options.Events = new JwtBearerEvents
    {
        ...
        {
            Console.WriteLine($"[LocalAuth FAIL - Sales] {context.Exception.Message}");
            return Task.CompletedTask;
        },
        OnTokenValidated = context =>
        {
            Console.WriteLine($"[LocalAuth OK - Sales] Token validado con éxito");
            return Task.CompletedTask;
        }
    };
})
...
.AddJwtBearer("SupabaseAuth", options =>
{
    ...
    options.Events = new JwtBearerEvents
    {
        OnTokenValidated = context =>
        {
            ...
            Console.WriteLine($"[SupabaseAuth OK - Sales] Usuario autenticado y autorizado.");
            return Task.CompletedTask;
        },
        OnAuthenticationFailed = context =>
        {
            Console.WriteLine($"[SupabaseAuth FAIL - Sales] {context.Exception.Message}");
            return Task.CompletedTask;
        }
    };
});
